...
onMounted(async () => {
  ...
  if (!slug) {
    console.error("Category Slug is missing from route params!");
    // Rozważ przekierowanie lub pokazanie błędu użytkownikowi
    // return router.push('/'); // Przykładowe przekierowanie
    return;
  }

  await testStore.initTest(slug);
});
...
useHead(() => ({
  title: `Egzamin - ${testStore.categoryName || categorySlugFromRoute.value} | SiperPrawko`,
}));
</script>
